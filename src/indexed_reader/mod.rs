...
//!
//! 1. **Header.** The first `HEADER_SCAN_LIMIT` bytes are searched for `%PDF-`. The match offset becomes the index's
//!    *source origin*, so a file with junk prepended is addressed by its own logical offsets, exactly as the eager
//!    reader addresses it.
//! 2. **`startxref`.** The last `TAIL_SCAN_LIMIT` bytes are read once and searched backwards for `%%EOF`, then for
//!    `startxref` before it.
//! 3. **Cross-reference walk.** Each section is parsed from a window that starts at `XREF_INITIAL_WINDOW` and grows to
//!    at most `XREF_WINDOW_LIMIT`; xref streams decompress under `XREF_DECOMPRESSED_LIMIT`. The walk follows `/Prev`
//!    until a revision repeats, bounded by `MAX_XREF_REVISIONS` revisions and `MAX_XREF_ENTRIES` entries. Newer
//!    revisions win. Within one revision a `/XRefStm` supplement takes precedence over the classic section it
//!    supplements, because a hybrid-reference file is required to mask its compressed objects as free in the classic
//!    table (ISO 32000-1, 7.5.8.4).
//!
...
//!
//! - [`resolve_object`](IndexedReader::resolve_object) — one owned [`Object`]. For a normal entry the framer probes at
//!   most `INDIRECT_HEADER_LIMIT` bytes for the `N G obj` header, then grows a window from `INITIAL_OBJECT_WINDOW` in
//!   `OBJECT_GROWTH_CHUNK` steps until the object parses or the per-object cap is reached.
//! - [`resolve_object_shared`](IndexedReader::resolve_object_shared) — `Arc<Object>` through the per-reader object
//!   cache. Concurrent calls for one id are single-flighted: one thread does the work, the others wait on the same
//!   cell.
//! - [`resolve_many_shared`](IndexedReader::resolve_many_shared) — a batch. Compressed ids are grouped by their
//!   `/ObjStm` container so each container is fetched, decoded and header-indexed once per call rather than once per
...
//! # Memory bounds
//!
//! - **No whole-file buffer.** [`FileSource`](crate::source::FileSource) satisfies every read positionally. Nothing in
//!   this module ever asks a source for its entire contents. [`BytesSource`](crate::source::BytesSource) exists for
//!   callers who already hold the bytes.
//! - **Physical reads are split.** Through `CachedSource` every request is decomposed into aligned chunks of
//!   `SOURCE_CHUNK_BYTES` (64 KiB), so one logical read of a large window is many small reads against a bounded pool
//!   rather than one large allocation.
//! - **Caches are per reader and budgeted.** [`IndexedReaderCacheOptions`] states a total byte and entry budget, split
//!   one quarter to source chunks, one half to resolved objects and the remainder to decoded object streams. The object
//!   cache shards (up to `SHARED_OBJECT_MAX_SHARDS`, never below `SHARED_OBJECT_MIN_ENTRIES_PER_SHARD` entries per
//!   shard) so admission does not serialize on one lock. Eviction is best effort by construction: entries a caller
//!   still holds are skipped, and a pass gives up after `MAX_PINNED_EVICTION_SCAN` non-evictable candidates rather than
//!   block. Cache pressure decides *retention*, never whether a value resolves — a reader opened without a cache budget
//!   resolves the same values, just without reuse.
//! - **Allocation is charged before it happens.** The preflight path reserves against a [`ScalarResolutionPermit`]
//!   before reading, so a declared length that would exceed the permit fails as
//!   [`ScalarResourceLimit`](IndexedReaderError::ScalarResourceLimit) instead of being allocated and then rejected.
//! - **Reported, not guessed.** [`IndexedReader::index_stats`] and [`IndexedReader::cache_stats`] report retained
//!   bytes, peaks and hit rates, which is how the bounds above are tested rather than asserted.
//!
...
//!
//! - **Per-object decode cap.** [`ObjectLimitExceeded`](IndexedReaderError::ObjectLimitExceeded) carries an
//!   [`ObjectLimitProvenance`] distinguishing "the framer still wanted more input at the configured maximum" from "the
//!   source ran out" from "checked arithmetic could not represent the window". Bounded caches use that field; without
//!   it they would have to parse the display string.
//! - **Page-tree depth and count.** The page-tree walk is iterative and bounded by
//!   [`IndexedReaderOptions::page_tree_depth`] and [`IndexedReaderOptions::max_pages`], with a per-node dereference
//!   budget of `PAGE_TREE_DEREFERENCE_LIMIT`. A `/Kids` cycle terminates as
//!   [`PageTreeDepthLimitExceeded`](IndexedReaderError::PageTreeDepthLimitExceeded) rather than recursing.
//! - **Unsupported object-stream and encoded-stream shapes.** A compressed entry whose container is not a stream, or
//!   whose member index does not decode, is reported against both ids
//!   ([`ObjectStreamMember`](IndexedReaderError::ObjectStreamMember)). An encoded payload that is encrypted, or
//!   otherwise not exposable as plaintext, refuses at descriptor-open time with
//!   [`Protected`](IndexedStreamReadError::Protected) instead of handing back ciphertext; such objects remain available
//!   through ordinary resolution, which decrypts. A payload with no provable span refuses with
//!   [`LengthUnavailable`](IndexedStreamReadError::LengthUnavailable).
//! - **Encryption.** A document needing a password fails as [`PasswordRequired`](IndexedReaderError::PasswordRequired)
//!   at open; a wrong one as [`InvalidPassword`](IndexedReaderError::InvalidPassword). [`IndexedReaderOptions`]'s
//!   `Debug` redacts the password value.
//!
...
//!
//! - The catalog must be **provable**. If the recovered `/Root` is not itself an object the scan found as a normal
//!   header, the original cross-reference error is returned unchanged, so a caller's own fallback sees exactly what it
//!   saw before. Opening a document that resolves to nothing would be strictly worse than reporting the failure.
//! - A trailer is never **synthesized over encryption**. If the body mentions `/Encrypt` anywhere, synthesis is
//!   forbidden, because a synthesized trailer cannot carry the encryption dictionary and the result would silently be
//!   read as plaintext.
//! - The outcome is **provenance-marked**: [`IndexedReaderIndexStats::recovered`] is `true` only for an index built
...
//!
//! - Resolving an id yields the same value whether it was cached, resolved concurrently by another thread, or resolved
//!   by a reader with no cache.
//! - [`resolve_many_shared`](IndexedReader::resolve_many_shared) returns first-occurrence order of the requested ids
//!   regardless of internal completion order, and deduplicates.
//! - Where two structures disagree — duplicate object-stream members, a `/XRefStm` supplement against its classic
//!   section, a `/Prev` chain — the precedence rule is fixed and documented at the site, not left to whichever parse
//!   finished first.
//! - Only source I/O failures are treated as retryable: they are shared with callers already waiting on the cell and
//!   then discarded, so a later call can try again. Deterministic failures stay cached, because re-deriving them would
//!   cost the same work and give the same answer.
//!
