...
mod selected_member_tests {
    ...
    #[test]
    fn unbounded_decompression_failure_keeps_selected_raw_byte_fallback() {
        let mut dict = Dictionary::new();
        dict.set("Type", "ObjStm");
        dict.set("N", 1i64);
        dict.set("First", 4i64);
        ...
        // Upstream's eager ObjectStream constructor now propagates decode
        // failures. The selected-member path deliberately retains its bounded,
